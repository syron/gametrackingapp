...
// Note: This separation is not strictly necessary, but I really like
// this pattern for re-use and readability.
etagUtil.factory('etagLocalStorageCacheFn', function () {
...
angular.module('AngularEtag', ['etagUtil'])
    ...
    .provider('etagLocalStorageCache', function () {
        ...
        // We don't have any angular magic yet to pull in dependencies.
        // So we have to manually do it with angular.injector
        var injector = angular.injector(['etagUtil']);

        //With the injector we can 'get' services from the other module
        // like the caching etags in local storage function
        this.$get = function () {
            return injector.get('etagLocalStorageCacheFn');
        };
    })

    // Now we're going to create our $http drop in replacement
    .config(function ($provide, etagLocalStorageCacheProvider) {

        //Using the decorator we add an initialization method
        //to $http (the $delegate)
    ...
    .factory('ehttp', function ($http, $window, $q) {
        ...
        // however, $window and $q are defined here.
